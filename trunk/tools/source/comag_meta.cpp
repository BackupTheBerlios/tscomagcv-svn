...
namespace comag {
    ...
    void meta::reset(void) {
	std::tm tm(*std::gmtime(NULL));
	title_type = TT_STATION;
	start_time.hour   = tm.tm_hour;
	start_time.minute = tm.tm_min;
	start_time.second = tm.tm_sec;
	start_date.year   = tm.tm_year + 1900;
	start_date.month  = tm.tm_mon + 1;
	start_date.day    = tm.tm_mday;
	start_date.wday   = tm.tm_wday;
	duration.hour     = 0;
	duration.minute   = 0;
	duration.second   = 0;
	packets = 0;
	...
	buffer[0x64] = 0x01;
	buffer[12] = title_type;
	strncpy( (char *)buffer + 0x0d, title.data(), 62 );
	buffer[0x4b] = start_time.hour;
	buffer[0x4c] = start_time.minute;
	buffer[0x4d] = start_time.second;
	buffer[0x50] = start_date.year & 0xff;
	buffer[0x51] = (start_date.year >> 8) & 0xff;
	buffer[0x52] = start_date.month;
	buffer[0x53] = start_date.day;
	buffer[0x54] = start_date.wday;
	buffer[0x58] = packets & 0xff;
	buffer[0x59] = (packets >> 8) & 0xff;
	buffer[0x5a] = (packets >> 16) & 0xff;
	buffer[0x5b] = (packets >> 24) & 0xff;
	buffer[0x5c] = duration.hour;
	buffer[0x5d] = duration.minute;
	buffer[0x5e] = duration.second;
	buffer[0x60] = service;
	buffer[0x68] = pid & 0xff;
	buffer[0x69] = ( pid >> 8 ) & 0xff;
	buffer[0x6c] = timer & 0xff;
	buffer[0x6d] = ( timer >> 8 ) & 0xff;
	buffer[0x6e] = ( timer >> 16 ) & 0xff;
	buffer[0x6f] = ( timer >> 24 ) & 0xff;
	...
	if ( memcmp( buffer,"\x05\x00\x72\x6a\x07\x00\x00\x00\x00\x00\x00\x00", 
	    ...
	    throw failure( "Header or reserved bytes not valid" );
	switch( buffer[12] ) {
		...
		break;
	    default:
		throw failure( "Station type is neither TRANSMISSION nor STATION" );
	}
	...
	if ( ( ( start_time.hour   = buffer[0x4b] ) > 24 ) ||
	    ...
	    throw failure( "Start time not valid" );
	if ( ( ( start_date.year = buffer[0x50] | (buffer[0x51] << 8) ) < 1900 ) ||
	    ...
	    throw failure( "Start date not valid" );
	...
	switch( buffer[0x60] ) {
		...
		break;
	    default:
		throw failure( "Service is neither TV nor RADIO" );
	}
